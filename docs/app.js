...
const SEED_POSTS = [
  ...
  { id: "p_ascent_sage", type: "video", title: "亚海悬城 · 贤者 B 大非常规天空位", summary: "中厅后门贴箱跳投，冰墙卡在拱门上沿。", body: "墙要贴上沿，太低会被 B 楼打穿。", map: "ascent", agent: "sage", ability: "冰墙", side: "进攻", site: "B", difficulty: "困难", likes: 932, favs: 411, comments: [], views: 12840, author: "瞬点官方" },
  { id: "p_icebox_omen", type: "guide", title: "森寒冬港 · 暗影厨房单向烟笔记", summary: "烟的下沿卡窗沿，你蹲着能看见脚。", body: "大多数人高度不对。原则：下沿卡窗沿。", map: "icebox", agent: "omen", ability: "暗影之幕", side: "防守", site: "B", difficulty: "简单", likes: 1503, favs: 977, comments: [{ user: "瞬点官方", body: "单向高度讲得很清楚。" }], views: 19021, author: "烟位研究员" },
...
function render() {
  ...
  if (path === "/products") {
    app.innerHTML = `<h1 style="font-size:28px">产品</h1>
      <p class="muted">瞬点先作为独立产品上线。后续再并进主站软件产品列。</p>
  ...
  if (path === "/" ) {
    app.innerHTML = `<section class="grid hero" style="align-items:center;gap:32px;margin-bottom:40px">
      <div>
        ...
        <p class="muted">瞬点先作为独立产品上线。上传教学视频和图文笔记，别人可以评论、点赞、收藏、分享。</p>
  ...
  if (path.startsWith("/posts/")) {
    ...
    document.getElementById("share").onclick = async () => {
      ...
      try { await navigator.clipboard.writeText(`【瞬点】${p.title}\n${url}`); alert("分享卡片已复制"); } catch { alert(url); }
    };
  ...
  if (path === "/login") {
    app.innerHTML = `<form class="panel clip" style="max-width:420px;margin:40px auto;padding:24px" id="lf">
      <h1 style="font-size:28px">登录瞬点</h1>
